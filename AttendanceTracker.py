...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgresr'
...
def send_verification_email(user):
    try:
        msg = Message('Welcome to XXYYZZ!', sender=app.config['MAIL_USERNAME']', recipients=[user.email])
...
def send_leave_email(leaves, user):
    try:
        msg = Message('Leave Application', sender=app.config['MAIL_USERNAME'], recipients=[user.email])
...
def send_leave_approval_email(leaves, user):
    try:
        msg = Message('Leave Application Approval', sender=app.config['MAIL_USERNAME'], recipients=[user.email])
